...
public class StringUtil {
	...
	public static String capitalizeEveryWord(String source, List<Delimiter> delimiters, Locale locale) {
		...
		// If Locale specified, i18n toLowerCase is executed, to handle specific
		// behaviors (eg. Turkish dotted and dotless 'i')
		if (locale != null)
			...
			if (!isSurrogate(chars[i]) && !Character.isLetter(chars[i])) {
				...
				for (Delimiter delimiter : delimiters) {
					if (delimiter.getDelimiter() == chars[i]) {
						...
						if (delimiter.capitalizeBefore() && i > 0 && Character.isLetter(chars[i - 1])
								&& !isSurrogate(chars[i - 1])) { // previous character is a Letter and I have to
																	// capitalize it
							chars[i - 1] = Character.toUpperCase(chars[i - 1]);
						}
						if (delimiter.capitalizeAfter() && i < chars.length - 1 && Character.isLetter(chars[i + 1])
								&& !isSurrogate(chars[i + 1])) { // next character is a Letter and I have to capitalize
																	// it
							chars[i + 1] = Character.toUpperCase(chars[i + 1]);
						}
						break;
	...
	static enum Behavior {
		CAPITALIZE_AFTER_MARKER(0), CAPITALIZE_BEFORE_MARKER(1), CAPITALIZE_BEFORE_AND_AFTER_MARKER(2);
